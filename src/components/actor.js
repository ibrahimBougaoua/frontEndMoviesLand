import React, { Component } from "react";
import Links from "./links";
import Nav from "./nav";
import axios from 'axios';
import Post from "./post";

// handle button click of login form
async function ifAbonner(actor) {
  try {
    const response = await axios.get('http://127.0.0.1:5000/movie/check/abonnement/user/' + localStorage.getItem('email') + '/actor/' + actor);
    console.log(response);
    return response;
  } catch (error) {
    console.error(error);
  }
}

// handle button click of login form
async function actorMovies(actor) {
    try {
      const response = await axios.get('http://127.0.0.1:5000/movie/abonnements/actor/' + actor);
      console.log(response);
      return response;
    } catch (error) {
      console.error(error);
    }
}

export default class Actor extends Component {

state = { checked: false,enable: false,movie: [] }


componentDidMount =()=>{

  ifAbonner(this.props.match.params.actor).then(response => {
if(response.data === 'yes'){
  this.setState({
    enable: true
  });
}
  });
	actorMovies(this.props.match.params.actor).then(response => {
	  this.setState({
		movie: response.data
	  });
	  console.log(this.props.match.params.actor)
	});
  }


render() {

// handle button click of login form
const new_Abonnement = () => {
    axios.get('http://127.0.0.1:5000/movie/abonnement/new', {params : {_email:  localStorage.getItem('email'),  _actor: this.props.match.params.actor}})
    .then(function (response) {
      // setter
      //const token = localStorage.setItem('token', response.data.access_token)
      //const user = localStorage.setItem('user', response.data.user)
      // route for profile
      console.log(response)

    }).catch(function (error) {
      console.log('ibrahim => ' + error);
    });

}


// handle button click of login form
const delete_Abonnement = () => {
    axios.get('http://127.0.0.1:5001/movie/abonnement/delete', {params : {_email:  localStorage.getItem('email'),  _author: this.props.match.params.author}})
    .then(function (response) {
      // setter
      //const token = localStorage.setItem('token', response.data.access_token)
      //const user = localStorage.setItem('user', response.data.user)
      // route for profile
      console.log(response)

    }).catch(function (error) {
      console.log('ibrahim => ' + error);
    });
}

const tags = ['adventure','animation','children','comedy','fantasy','children','fantasy','romance','drama','action','crime','thriller','horror','mystery','sci-Fi','documentary','imax'];
    

return (
    <div>

<div className="container">

<Nav name="Actor"></Nav>

<div className="card mb-5 border-0 rounded-0 shadow-sm bg-light">
	<div className="row no-gutters">
		<div className="col-md-3">
			<img src="data:image/jpeg;base64," class="card-img rounded-0" alt="..." />
		</div>
		<div className="col-md-9">
			<div className="card-body">
				<h5 className="card-title">{ this.props.match.params.actor }</h5>
      
  {
  this.state.enable
    ? <p className="card-text"><button type="submit" className="btn btn-sm btn-secondary disabled" onClick={delete_Abonnement}>s'abonner</button></p>
    : <p className="card-text"><button type="submit" className="btn btn-sm btn-danger" onClick={new_Abonnement}>s'abonner</button></p>
  }  
  		</div>
		</div>
	</div>
</div>
</div>

<Post name="all movies" link="/account/single/" elements={this.state.movie}></Post>

<Links name="Tags" path="/account/tag/" elements={tags}/>

    </div>
);
}

}