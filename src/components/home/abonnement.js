...
async function actors() {
    try {
      const response = await axios.get('http://127.0.0.1:5000/movie/abonnements/user/' + localStorage.getItem('email'));
...
// handle button click of login form
async function movies(user_id) {
    try {
      const response = await axios.get('http://127.0.0.1:5000/movie/abonnements/' + user_id);
      console.log(response);
      return response;
    } catch (error) {
      console.error(error);
...
export default class Abonnement extends Component {
    ...
    state = {
      ...
      componentDidMount =()=>{
        actors().then(response => {
          this.setState({
            actors: response.data
          });
        });
        movies(1).then(response => {
          this.setState({
...
render() {
	...
	const actors = this.state.actors.map((element) => 
	...
	<div className="row no-gutters">
		...
		<div className="col-md-8">
			<div className="pl-2">
                ...
                <Link className="btn btn-success btn-sm position-relative" to={"/movie/actor/" + element} style={{bottom: "3px"}}>Show more</Link>
			</div>
		</div>
	</div>
    </div>
...
return (
    ...
    <Post name="Abonnements" link="/movie/single/" elements={this.state.movies}></Post>
    </div>
