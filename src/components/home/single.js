...
state = { visibleComponent: false,value: 0,movie: [],related: [] };
...
<div className="card mb-5 border-0 rounded-0 shadow-sm bg-light">
	...
	{element[7].split(',').map((element) => <Link className="btn btn-sm btn-outline-info mr-2 mt-2 mb-2" to={"/movie/tag/" + element}>{element}</Link>)}<br />
...
<div className="card mb-5 border-0 rounded-0 shadow-sm bg-light">
	<div className="row no-gutters">
        ...
        <p className="text-sm-left" style={{fontSize:"1vw"}}>{element}</p>
				<Link className="btn btn-sm btn-outline-info mr-2" to={"/movie/actor/" + element}>More</Link>
			</div>
		</div>
	</div>
</div>
...
<Post name="You May Also Like" link="/movie/single/" elements={this.state.related}></Post>
